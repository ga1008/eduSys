      ...
      <template v-if="currentAssignment">
        <div class="dialog-content">
          ...
          <div class="dialog-info-row"
            ...
            <span class="info-value">
              <el-tag :type="getScoreTagType(currentAssignment.score)">{{ currentAssignment.score }} 分</el-tag>
            </span>
          </div>

          <div class="dialog-info-row" v-if="currentAssignment.teacher_comment && !currentAssignment.is_returned">
            <span class="info-label">评语:</span>
            <span class="info-value">{{ currentAssignment.teacher_comment }}</span>
          </div>


          <div class="mt-6">
            <div class="font-bold mb-2">作业描述:</div>
            <div class="description-box">{{ currentAssignment.description }}</div>
          </div>
        </div>
      </template>
      <template #footer>
  ...
  try {
    ...
    const coursePromises = courseList.map(async (course) => {
      try {
        ...
        // Ensure assignments have necessary properties, defaulting if not present
        ...
        return {...course, assignments: []} // Return course with empty assignments on error
      }
    })
...
const sortCoursesAndAssignments = () => {
  ...
  courses.value.forEach(course => {
    course.assignments.sort((a, b) => {
      ...
      if (aOverdue !== bOverdue) return aOverdue ? 1 : -1; // 未截止的在前

      const aEffectivelySubmitted = isEffectivelySubmitted(a);
      const bEffectivelySubmitted = isEffectivelySubmitted(b);
      if (aEffectivelySubmitted !== bEffectivelySubmitted) return aEffectivelySubmitted ? 1 : -1; // 未提交的在前 (among non-overdue)

      return dayjs(a.due_date).valueOf() - dayjs(b.due_date).valueOf(); // 按截止日期升序 (越早截止越前)
    });
  });
};
...
const setActiveCourseFromRoute = () => {
  // Try to set from route param first
  ...
  // Then, if no match or no param, try the first course that has pending assignments
  ...
  // Fallback to the first course if it exists
...
const previewAssignment = (assignment) => {
  currentAssignment.value = {...assignment} // Create a copy to avoid reactivity issues if needed
...
const getAssignmentStatusText = (assignment) => {
  if (isOverdue(assignment)) {
    ...
    if (assignment.submitted) {
      return (assignment.score !== null && typeof assignment.score !== 'undefined') ? `已截止 (已评分 ${assignment.score})` : '已截止 (已提交)';
    }
    return '已截止 (未提交)';
  }
  // Not overdue
  if (assignment.is_returned) return '已退回 (待重交)';
  if (assignment.submitted) { // Effectively submitted or graded
    return (assignment.score !== null && typeof assignment.score !== 'undefined') ? `已评分 (${assignment.score})` : '已提交 (待批改)';
  }
  return '待提交';
};
...
const getAssignmentStatusClass = (assignment) => {
  ...
  if (assignment.is_returned) return 'status-returned'; // For "已退回 (待重交)"
  if (assignment.submitted) { // Effectively submitted or graded
    // Could add a specific 'status-graded' if desired
    return 'status-submitted';
  }
  return 'status-pending'; // For "待提交"
};
...
const filteredCourses = computed(() => {
  return courses.value.map(course => {
    const filteredAssignments = course.assignments.filter(asg => {
      ...
      // Pending: Not overdue AND ( (not submitted at all) OR (submitted but returned for resubmission) )
      const isPending = !overdue && (!asg.submitted || asg.is_returned);

      if (statusFilter.value === 'all') return true;
      if (statusFilter.value === 'pending') return isPending;
      // Submitted: Not overdue AND (submitted and not returned)
      if (statusFilter.value === 'submitted') return !overdue && effectivelySubmitted;
...
watch(
    ...
    (newId) => {
      if (newId) {
        ...
        setActiveCourseFromRoute(); // Recalculate if param is removed or changed
      }
    }
)
</script>
...
/* You might want a different color for "graded" if you distinguish it more */
.status-graded {
...
.assignment-description {
  ...
  font-size: 13px;
  margin-bottom: 12px;
  min-height: 38px; /* approx 2 lines */
  line-height: 1.4;
  overflow: hidden;
  display: -webkit-box;
  -webkit-line-clamp: 2;
...
.custom-collapse .el-collapse-item { /* Add margin between collapse items */
  margin-bottom: 10px;
}
